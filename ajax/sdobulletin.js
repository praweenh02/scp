...
function save_data() {
    ...
    if ($('#form-question').valid()) // check if form is valid
    {
        ...
        $.ajax({
            url: SITEROOT + 'super-admin/sdobulletin/save_data',
            type: "post",
            ...
            success: function (response) {
                ...
                if ($.trim(data_obj.status) === 'success') {
                    ...
                    setTimeout(function () {
                        ...
                        window.location.href = 'super-admin/sdobulletin/';
                    }, 2000);
...
function deleteData(id) {
    bootbox.confirm("Are you sure you want to delete this row?", function (result) {
        if (result == true) {
            ...
            $.post(SITEROOT + 'super-admin/sdobulletin/delete_data', { id: id, csrf_token: csrf_token }, function (data) {
                deleteData
                if ($.trim(data) != 'success') {
                    ...
                    $.toast({
                        heading: 'Deleted',
                        text: 'SDO Bulletin deleted successfully.',
                        icon: 'error',
                        position: 'bottom-center'
                    });
                    setTimeout(function () {
