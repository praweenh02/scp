...
function save_data()
{
    ...
    if ($('#form-whatsnew').valid()) // check if form is valid
    {
        var whatsnew_id = $("#whatsnew_id").val();

        var form = $('#form-whatsnew')[0]; 
        formData = new FormData(form);
        formData.append('whatsnew_id',$("#whatsnew_id").val());
          //formData.append('faq_answer', CKEDITOR.instances['faq_answer'].getData());
          //formData.append('faq_answer_hindi', CKEDITOR.instances['faq_answer_hindi'].getData());
           ...
           $.ajax({
            url: SITEROOT +'super-admin/whatsnew/save_data',
            type: "post",
            ...
            success: function (response)
            {
                ...
                if ($.trim(data_obj.status) === 'success') {
                    ...
                    setTimeout(function () {
                       ...
                       window.location.href='super-admin/whatsnew/';
                   }, 2000);
   ...
   function deleteData(id)
   {
    bootbox.confirm("Are you sure you want to delete this row?", function(result) {
        ...
        { 
         ...
         $.post(SITEROOT + 'super-admin/whatsnew/delete_data', {id: id, csrf_token:csrf_token}, function (data) {deleteData
             if ($.trim(data) != 'success') {
                        ...
                        $.toast({
                            heading: 'Deleted',
                            text: 'Whats new deleted successfully.',
                            icon: 'error',
                            position : 'bottom-center' 
                        });
                        setTimeout(function() {
