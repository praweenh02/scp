                ...
                <div class="panel-body">
                    <div class="tab-content">
                        <div class="tab-pane active " id="home-2">
                            <form class="cmxform form-horizontal adminex-form" id="form-circular" name="form-circular" method="post" enctype="multiple/form-data">
                                <?php  $csrf = array(
                                'name' => $this->security->get_csrf_token_name(),
                                'hash' => $this->security->get_csrf_hash());
                                ?>
                                <input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" />
                                <input type="hidden" name="circulars_id" id="circulars_id" value="<?=$result->circulars_id;?>">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group row">
                                        <label for="cname" class="control-label col-lg-3">Circular Title *  </label>
                                        <div class="col-lg-9">
                                            <input type="text" name="circulars_title" id="circulars_title" class="form-control " value="<?=$result->circulars_title;?>" required>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="cname" class="control-label col-lg-3">Circular  File  </label>
                                        <div class="col-lg-9">
                                            <input type="file" name="circulars_file" id="circulars_file" class="form-control" value="" >
                                            <?php
                                            if($result->circulars_file)
                                            {
                                            echo '<a target="_blank" href="uploads/circulars/'.$result->circulars_file.'" >File</a>';
                                            }
                                            ?>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-lg-9">
                                            <select  name="status" id="status" class="form-control">
                                                ...
                                                <option value="Y" <?=$result->circulars_status=='Y'? 'selected':'' ;?>>Active</option>
                                                <option value="N" <?=$result->circulars_status=='N'? 'selected':'' ;?>>Inactive</option>
                                            </select>
                                        </div>
                                    </div>
                                    <?php }?>
                                    <div class="form-group row">
                                        <div class="pull-right">
                                            ...
                                            <button class="btn btn-success btn-sm" onclick="save_circular();" type="button">Save changes</button>
                                        </div>
                                    </div>
                                </div>
                            </form>
