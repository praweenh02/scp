...
class Groupmanager_model extends CI_model
{
	...
	public function getAllWorkikgItem($group_id,$user_id)
	{
		...
		$query = $this->db->select('work_item.*, groups.group_title')->join('groups','work_item.group_id=groups.group_id','left')->where('work_item.group_id',$group_id)->order_by('work_item.workitem_id','DESC')->get('work_item')->result();
		return $query;
