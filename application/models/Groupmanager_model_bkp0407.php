...
class Groupmanager_model extends CI_model
{
	...
	public function getAllMemberDocuments($group_id)
	{
		...
		$query = $this->db->select('group_contributions.*, groups.group_title,users.name,users.surname')->join('groups','group_contributions.group_id = groups.group_id','left')->join('users','group_contributions.user_id=users.user_id','left')->where('group_contributions.group_id',$group_id)->where('group_contributions.delete','Y')->where('group_contributions.file_status','uploaded')->or_where('group_contributions.file_status','re-uploaded')->get('group_contributions');
		return $query->result();
	}
